...
export interface Interface {
	...
	) => Effect.Effect<GenerateResult, DocumentAgentError>;
	readonly patch: (
		path: string,
		sectionTitle: string,
...
export class Service extends Context.Tag("DocumentAgent")<
	Service,
	Interface
>() {}
...
export const layer = Layer.effect(
	...
	Effect.gen(function* () {
		...
		const generate = Effect.fn("Agent.generate")(function* (
			request: DocumentRequest,
		) {
			...
			const main = Effect.gen(function* () {
				yield* log("info", "request", "요청 분석 중...", {
					prompt: normalized.prompt,
				});

				const plan = yield* llm.planDocument(normalized);
				yield* repo.savePlan(documentId, plan);
				yield* log("info", "planner", "아웃라인 생성 완료", {
					title: plan.title,
					sections: plan.sections.length,
				});

				const sectionDrafts = yield* writeSections(
					documentId,
					normalized,
					plan.sections,
					plan,
				...
				const intro = yield* llm.writeFrame({
					kind: "intro",
					request: normalized,
					plan,
					sectionDrafts,
				});
				const conclusion = yield* llm.writeFrame({
					kind: "conclusion",
					request: normalized,
					plan,
					sectionDrafts,
				...
				let edited = yield* llm.editDocument({
					request: normalized,
					plan,
					intro,
				...
				yield* log("info", "editor", "문서 편집 완료", {
					words: wordCount(edited),
				});

				let review = normalizeReviewReport(
					yield* llm.reviewDocument({
						request: normalized,
						plan,
						markdown: edited,
					}),
				...
				yield* repo.completeDocument(
					documentId,
					plan.title,
					edited,
					normalized.outputPath,
				);
				yield* log("info", "final", "문서 생성 완료", {
		...
		const patch = Effect.fn("Agent.patch")(function* (
			...
			const main = Effect.gen(function* () {
				...
				const currentSection = yield* store.extractSection(
					original,
					sectionTitle,
				);
				const replacement = yield* llm.patchExistingSection({
					documentMarkdown: original,
					sectionTitle,
					request,
				});
				const nextMarkdown = yield* store.replaceSection(
					original,
					sectionTitle,
					replacement,
				);
...
function writeSections(
	...
	return Effect.gen(function* () {
		...
		for (const batch of batches) {
			yield* log("info", "executor", "섹션 초안 작성 중...", {
				batch: batch.map((s) => s.title),
			});

			const drafts = yield* Effect.all(
				batch.map((section) =>
					Effect.gen(function* () {
						const draft = yield* llm.writeSection({
							request,
							plan,
							section,
		...
		return sections
			.map((section) =>
				completed.find((draft) => draft.sectionId === section.id),
			)
...
function patchWeakSections(
	...
	return Effect.gen(function* () {
		...
		for (const sectionTitle of review.weakSections) {
			const planSection = sections.find(
				(section) => section.title === sectionTitle,
			);
			const currentDraft = nextDrafts.find(
				(draft) => draft.title === sectionTitle,
			);
			...
			const index = nextDrafts.findIndex(
				(draft) => draft.sectionId === patched.sectionId,
			);
