...
export abstract class OpenAiCompatibleClientBase {
	...
	planDocument(
		...
		return Effect.tryPromise({
			try: () => this.planDocumentPromise(request),
				...
				error instanceof ExternalDependencyError
					? error
					: new ExternalDependencyError(
							`${this.providerLabel} plan 생성 실패`,
							error,
						),
		});
	}
		...
		return Effect.tryPromise({
			try: () =>
				this.parseStructured(
					buildReviewDocumentPrompt(input),
					reviewSchema,
					"review_report",
				),
			catch: (error) =>
		...
		return Effect.tryPromise({
			try: () => this.generateMarkdown(buildPatchExistingSectionPrompt(input)),
				...
				error instanceof ExternalDependencyError
					? error
					: new ExternalDependencyError(
							`${this.providerLabel} 기존 섹션 패치 실패`,
							error,
						),
		});
	}
		...
		try {
			...
			if (!text) {
				throw new WorkflowError(
					`${this.providerLabel}가 Markdown 응답을 비워서 반환했습니다.`,
				);
			}
			...
			throw new ExternalDependencyError(
				`${this.providerLabel} Markdown 생성에 실패했습니다.`,
				error,
			);
		}
	}
	...
	protected async parseStructured<T extends z.ZodTypeAny>(
		...
		try {
			...
			if (!parsed) {
				throw new WorkflowError(
					`${this.providerLabel} 구조화 응답을 파싱하지 못했습니다.`,
				);
			}
			...
			throw new ExternalDependencyError(
				`${this.providerLabel} 구조화 응답 생성에 실패했습니다.`,
				error,
			);
		}
	}
