  ...
  Effect.sync(() => {
    const read = (path: string): Effect.Effect<string, FilePersistenceError> =>
      Effect.tryPromise({
        ...
        catch: (error) => new FilePersistenceError(`문서를 읽지 못했습니다: ${path}`, error),
      });

    const write = (path: string, markdown: string): Effect.Effect<void, FilePersistenceError> =>
      Effect.tryPromise({
        try: async () => {
        ...
        catch: (error) => new FilePersistenceError(`문서를 저장하지 못했습니다: ${path}`, error),
      });

    const extractSection = (markdown: string, sectionTitle: string): Effect.Effect<HeadingSection, InputError> =>
      Effect.sync(() => {
    ...
    const replaceSection = (markdown: string, sectionTitle: string, replacementMarkdown: string): Effect.Effect<string, InputError> =>
      Effect.sync(() => {
...
function extractSectionSync(markdown: string, sectionTitle: string): HeadingSection {
  const sections = parseSections(markdown);
  const match = sections.find((section) => section.title === sectionTitle);
  if (!match) {
    throw new InputError(`문서에서 섹션을 찾지 못했습니다: ${sectionTitle}`);
